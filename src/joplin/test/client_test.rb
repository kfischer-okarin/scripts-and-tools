...
class ClientTest < Joplin::TestCase
  ...
  def test_folders_returns_all_folders_with_parent_ids
    stub_request(:get, "http://localhost:41184/folders?token=#{@token}&page=1")
      .to_return(
        status: 200,
        body: JSON.generate({
          "items" => [
  ...
  def test_folders_paginates_when_has_more_is_true
    stub_request(:get, "http://localhost:41184/folders?token=#{@token}&page=1")
      .to_return(
        status: 200,
        body: JSON.generate({
          "items" => [{ "id" => "folder1", "title" => "First", "parent_id" => "" }],
          "has_more" => true
        }),
        headers: { "Content-Type" => "application/json" }
      )

    stub_request(:get, "http://localhost:41184/folders?token=#{@token}&page=2")
      .to_return(
        status: 200,
        body: JSON.generate({
          "items" => [{ "id" => "folder2", "title" => "Second", "parent_id" => "" }],
          "has_more" => false
        }),
        headers: { "Content-Type" => "application/json" }
      )
  ...
  def test_calls_logger_with_request_and_response
    stub_request(:get, "http://localhost:41184/folders?token=#{@token}&page=1")
      .to_return(
        status: 200,
        body: JSON.generate({
          "items" => [{ "id" => "f1", "title" => "Test", "parent_id" => "" }],
          "has_more" => false
        }),
        headers: { "Content-Type" => "application/json" }
      )
    ...
    assert_equal "/folders?token=#{@token}&page=1", logged[0][:request][:path]
